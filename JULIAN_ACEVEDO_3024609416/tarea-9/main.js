...
/*Hacer un programa que simule una peticion a un servidor, con delay de 2000 ms, donde se obtenga una
informacion de selecciones de la Copa América. Si el usuario general de la aplicación esta logueado (isLogged)
la promesa se resuelve con las selecciones (selecciones), si no, se rechaza con un mensaje diciendo 
"No estas logueado dentro de la app". Dentro de cada selección hay una propiedad código de país (code)
que servirá para obtener la informacion de los jugadores de esa selección, simulando otra peticion tambien
...
function obtenerJugadoresPorSeleccion(codigoPais) {
  return new Promise((resolve) => {
    setTimeout(() => {
      const jugadoresSeleccion = jugadores.filter(
...
obtenerSelecciones()
  ...
  .then((resultados) => {
    resultados.forEach((jugadores, index) => {
      console.log(
        `Jugadores de la selección ${selecciones[index].nombre}:`,
        jugadores
      );
    });
  })
