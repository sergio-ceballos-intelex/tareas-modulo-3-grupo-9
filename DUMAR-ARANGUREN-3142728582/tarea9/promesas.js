...
function obtenerSelecciones() {
  return new Promise((resolve, reject) => {
    ...
    if (isLogged){
      ...
      reject('no estas logueado en la app');
    }
     },2000);
...
function obtenerJugadoresPorSeleccion(codigoPais) {
  return new Promise((resolve) => {
    setTimeout(() => {
      const jugadoresFil = jugadores
      .filter(jugador => jugador.codigoPais === codigoPais)
      .map(jugador=>({id:jugador.id, nombreCompleto: `${jugador.nombre}${jugador.apellido}`}));
      resolve(jugadoresFil);
    }, 5000);
  });
}
...
.then(jugadoresPorSeleccion => {
  const resultadoFinal = selecciones.reduce((acc, seleccion, index) => {
    acc[seleccion.codigoPais] = jugadoresPorSeleccion[index].reduce((obj, jugador) => {
      obj[jugador.id] = jugador.nombreCompleto;
      return obj;
    }, {});
    return acc;
  }, {});
