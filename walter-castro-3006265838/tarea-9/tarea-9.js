//! TAREA-9
/* Hacer un programa que simule una peticion a un servidor, con delay de 2000 ms, donde se obtenga una informacion 
de selecciones de la Copa América. Si el usuario general de la aplicación esta logueado (isLogged) la promesa se 
resuelve con las selecciones (selecciones), si no, se rechaza con un mensaje diciendo "No estas logueado dentro de la app". Dentro de cada selección hay una propiedad código de país (code) que servirá para obtener la informacion de los jugadores de esa selección, simulando otra peticion tambien con delay de 5000 ms que se resolvera con todos los jugadores dentro de la lista jugadores.
Notas:
...
function obtenerJugadoresPorSeleccion(codigoPais) {
  return new Promise((resolve) => {
    setTimeout(() => {    
      const jugaXPorSeleccion = jugadores.filter(
        (jugador) => jugador.codigoPais === codigoPais
      );
      resolve(jugaXPorSeleccion);
    }, 5000);
  });
}
